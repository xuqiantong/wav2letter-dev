...
def score(x, wts):
        return x["am_score"] + wts["tr"] * x["tr"] + wts["convlm"] * x["convlm"] + wts["len"] * x["wl_len"]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hyp', type=str, required=True, help="candidates beam dump path")
    parser.add_argument('--list', type=str, required=True, help="data list with original transcriptions")
    ...
    parser.add_argument('--search', action='store_true', help="search or not optimal weights of rescoring")
    parser.add_argument('--top', type=str, default="small", help="large beam or not, defines the topk set")
    parser.add_argument('--gridsearch', action='store_true', help="use grid search instead of random search")
    parser.add_argument('--new', action='store_true', help="new format")
    ...
    lm_files = {key: open(name, "r") for name, key in zip(
        [args.convlm, args.tr], ["convlm", "tr"]) if name != ""}
    with open(args.hyp, "r") as f:
        for line in f:
            ...
            assert len(non_transcription) == 5, non_transcription
            audio_id = non_transcription[0].strip()
            if not args.new:
                transcription = data[5:] #data[5].strip().split(" ")
            else:
            ...
            for key in ["convlm", "tr"]:
                if key in lm_files:
                    values_dict[key] = float(lm_files[key].readline().strip().split(" ")[1])
                else:
                    values_dict[key] = 0.
    ...
    best_wts = {name: float(val) for val, name in zip(args.in_wts.split(","), ["tr", "convlm", "len"])}
